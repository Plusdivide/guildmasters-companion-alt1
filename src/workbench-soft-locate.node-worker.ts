/**
 * Node worker_threads entry for workbench soft-locate.
 * Workbench-only — separate from bank-soft-locate.node-worker.ts.
 */
import { parentPort, workerData } from "node:worker_threads";
import { softLocateSprite } from "./workbench-soft-locate-core.ts";

if (!parentPort) throw new Error("workbench soft-locate node worker needs parentPort");
